...
void patchFonts(NSDictionary<NSString *, NSString *> *mainFonts, NSString *fontDefName) {
    RetributionLog(@"patchFonts called with fonts: %@ and def name: %@", mainFonts, fontDefName);

    if (!fontMap) {
        RetributionLog(@"Creating new fontMap");
    ...
    NSString *fontJson = [NSString stringWithContentsOfURL:[getPyoncordDirectory() URLByAppendingPathComponent:@"fonts.json"]
                                                 encoding:NSUTF8StringEncoding
                                                    error:nil];
    if (fontJson) {
        RetributionLog(@"Found existing fonts.json: %@", fontJson);
    }

    for (NSString *fontName in mainFonts) {
        NSString *url = mainFonts[fontName];
        RetributionLog(@"Replacing font %@ with URL: %@", fontName, url);
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:parentDir.path]) {
            RetributionLog(@"Creating parent directory: %@", parentDir.path);
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:fontCachePath.path]) {
            RetributionLog(@"Downloading font %@ from %@", fontName, url);
            NSData *data = [NSData dataWithContentsOfURL:fontURL];
            if (data) {
                RetributionLog(@"Writing font data to: %@", fontCachePath.path);
        ...
        if (fontData) {
            RetributionLog(@"Registering font %@ with provider", fontName);
            ...
            if (font) {
                ...
                if (existingFont) {
                    ...
                    if (!CTFontManagerUnregisterGraphicsFont(font, &unregisterError)) {
                        RetributionLog(@"Failed to deregister font %@: %@", (__bridge NSString *)postScriptName,
                ...
                if (CTFontManagerRegisterGraphicsFont(font, &error)) {
                    ...
                    RetributionLog(@"Successfully registered font %@ to %@", fontName, (__bridge NSString *)postScriptName);
                    ...
                    RetributionLog(@"Failed to register font %@: %@", fontName, errorDesc);
...
%ctor {
    @autoreleasepool {
        fontMap = [NSMutableDictionary dictionary];
        RetributionLog(@"Font hooks initialized");
        %init;
    }
}
