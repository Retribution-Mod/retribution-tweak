...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.customLoadUrl = [NSURL URLWithString:@"http://localhost:4040/retribution.js"];
    }
    return self;
}
...
- (BOOL)loadConfig {
    ...
    RetributionLog(@"Attempting to load config from: %@", loaderConfigUrl.path);
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:loaderConfigUrl.path]) {
        ...
        if (error) {
            RetributionLog(@"Error parsing loader config: %@", error);
            return NO;
        }
        ...
        if (json) {
            ...
            RetributionLog(@"Loader config loaded - Custom URL %@: %@",
    ...
    RetributionLog(@"Using default loader config: %@", self.customLoadUrl.absoluteString);
    return NO;
}
...
+ (instancetype)defaultConfig {
    ...
    config.customLoadUrl = [NSURL URLWithString:@"http://localhost:4040/retribution.js"];
    return config;
}

+ (instancetype)getLoaderConfig {
    RetributionLog(@"Getting loader config");
    ...
    RetributionLog(@"Couldn't get loader config");
